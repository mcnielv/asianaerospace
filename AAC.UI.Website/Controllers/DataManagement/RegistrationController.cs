...
namespace AAC.UI.Website.Controllers.DataManagement
{
    ...
    public class RegistrationController : Controller
    {
        ...
        public ActionResult LoadDetails( int id )
        {
            ...
            model = _registrationService.GetAll().Where( x => x.ID == id ).FirstOrDefault();
        ...
        {
            string msg = _registrationService.Create( model );
            return Json( new { Message = msg }, JsonRequestBehavior.AllowGet );
        }
        [HttpPost]
        public ActionResult Update( RegistrationViewModel model )
        {
            //update data
            string msg = _registrationService.Update( model );
            return Json( new { Message = msg }, JsonRequestBehavior.AllowGet );
        }
        [HttpPost]
        public ActionResult Delete( int id )
        {
            string msg = _registrationService.Delete( id );
            return Json( new { Message = msg }, JsonRequestBehavior.AllowGet );
        }
        #endregion
                ...
                bool _search, string searchField, string searchOper, string searchString )
        {
            ...
            IQueryable<RegistrationViewModel> registrations = _registrationService.GetAll();

            // If search, filter the list against the search condition.
            // Only "contains" search is implemented here.
            var filteredRoles = registrations;
            if ( _search )
            {
                switch ( searchOper )
                {
                    case "eq":
                        filteredRoles = registrations.Where( x => x.Name == searchString ).AsQueryable();
                        break;
                        //case "cn":
            ...
            var sortedRoles = SortIQueryable<RegistrationViewModel>( filteredRoles, sidx, sord ).AsEnumerable();
